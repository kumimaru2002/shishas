...
export function addFlavor(flavorData) {
    const now = new Date();
    const newFlavor = {
        id: generateId(),
        ...flavorData,
...
export function updateFlavor(id, updates) {
    const flavors = getFlavors();
...
export function searchFlavors(query) {
    if (!query.trim())
        return getFlavors();
    const flavors = getFlavors();
    const lowerQuery = query.toLowerCase();
    return flavors.filter(flavor => flavor.name.toLowerCase().includes(lowerQuery) ||
...
export function filterFlavorsByScore(minScore, maxScore = 5) {
    const flavors = getFlavors();
    return flavors.filter(flavor => flavor.score >= minScore && flavor.score <= maxScore);
}
export function filterFlavorsByShop(shopId) {
    const flavors = getFlavors();
    return flavors.filter(flavor => flavor.shopId === shopId);
}
export function sortFlavors(flavors, sortBy, order) {
...
export function restoreData(backup) {
    try {
        ...
        if (data.shops)
            saveToStorage(STORAGE_KEYS.SHOPS, data.shops);
        if (data.flavors)
            saveToStorage(STORAGE_KEYS.FLAVORS, data.flavors);
        if (data.settings)
