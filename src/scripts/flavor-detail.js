class FlavorDetailController {
  ...
  renderFlavorDetail() {
    ...
    // 編集リンク更新
    this.elements.editFlavorBtn.href = `../pages/flavor-edit.html?id=${this.flavor.id}`;
  ...
  renderShopInfo() {
    ...
    this.elements.shopLink.href = `../pages/shop-detail.html?id=${shop.id}`;
  }
  ...
  async executeDelete() {
    ...
    try {
      ...
      if (success) {
        this.hideDeleteModal();
        this.showSuccessAndRedirect('フレーバーを削除しました。', '../pages/flavor-list.html');
      } else {
